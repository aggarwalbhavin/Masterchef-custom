...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    function updatePool(uint256 _pid) public {
        ...
        Token.transferFrom(RewardWalletAddress, address(this), TokenReward);
    ...
    function deposit(
        ...
        if (_amount > 0) {
            ...
            if (address(pool.lpToken) == address(Token)) {
                // TokenTax in basis points
                uint256 TokenTax = (Token._reflectionFee() +
                    Token._burnFee() +
                    Token._marketingTokenFee() +
                    Token._marketingETHFee()).mul(100);
                uint256 transferTax = _amount.mul(TokenTax).div(10000);
    ...
    function setTokenReferral(ITokenReferral _TokenReferral)
        public
        onlyOwner
    {
